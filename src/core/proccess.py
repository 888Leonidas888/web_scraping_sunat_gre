...
def run_massive_query_process(token: str, ruc_receptor: str, emitter_rucs: List[str]):
    ...
    for f in range(mes_inicio, mes_final + 1):        
        dias_mes = calendar.monthrange(anio, f)[1]
        for i in range(1, dias_mes + 1):
            todayStr = datetime.now().replace(year=anio,month=f,day=i).strftime("%Y-%m-%d")
            ...
            for ruc_emisor in emitter_rucs:
                try:
                    ...
                    segundos = random.uniform(0.5, 2.5)
                    time.sleep(segundos)
